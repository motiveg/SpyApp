...
class AlphanumericCesarCipherTests: XCTestCase {
    ...
    func test_nonAlphaNumericInput() {
        let testMessage = "^_^"
        let testSecret = "5"
        let assertMessage = "Error: Expected output message doesn't match for non-alphanumeric characters in text field.\n"
        
        let encodeResult = cipher.encode(testMessage, secret: testSecret)
        XCTAssertEqual(encodeResult, CipherMessage.alphanumericOnly, assertMessage)
        
        let decryptResult = cipher.decrypt(testMessage, secret: testSecret)
        XCTAssertEqual(decryptResult, CipherMessage.alphanumericOnly, assertMessage)
    }
