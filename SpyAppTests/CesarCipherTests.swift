...
//  SpyAppTests
//
//  Created by Brian Casipit on 9/13/18.
...
class CesarCipherTests: XCTestCase {
    ...
    func test_resultingASCIICharLessThan32() {
        ...
        let testChar = String(UnicodeScalar(UInt32(testASCIIValue))!)
        let testSecret = "5"
        let assertMessage = "Error: Expected output message doesn't match for unseeable ascii characters.\n"
        
        let encodeResult = cipher.encode(testChar, secret: testSecret)
        XCTAssertEqual(encodeResult, CipherMessage.invisibleCharError, assertMessage)
        
        let decryptResult = cipher.decrypt(testChar, secret: testSecret)
